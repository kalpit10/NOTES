...
const userSchema = new mongoose.Schema({
  //new definition because of mongoose encryption
  email: String,
  password: String,
  googleID: {
    type: String,
...
passport.use(
  new GoogleStrategy(
    ...
    function (accessToken, refreshToken, profile, cb) {
      ...
      User.findOrCreate(
        {
          //we first find the google id of that profile if it is there then bingo! if not then create one.
          googleId: profile.id,
...
app.get("/list", function (req, res) {
  Item.find({}, function (err, foundItems) {
    if (foundItems.length === 0) {
      //if we have 0 items then we insert 3 new items in there..
...
app.get("/:customListName", function (req, res) {
  //if we want any list for user for eg. if he wants to work on localhost:3000/home he can just search that and start working on it.
  const customListName = _.capitalize(req.params.customListName);

  List.findOne({ name: customListName }, function (err, foundList) {
    //if there is a err then show err if there is no err then show the founded list
    if (!err) {
      if (!foundList) {
        //Create a new List
        const list = new List({
...
app.post("/list", function (req, res) {
  ...
  if (listName === "Today") {
    //if user wants to save element in default list then just res.redirect to default
    item.save();
    res.redirect("/list");
  } else {
    //if a user came from a custom list then we are going to add the new item to that list and then redirect that page
    List.findOne({ name: listName }, function (err, foundList) {
      //foundList is singular here cause findOne
...
app.post("/delete", function (req, res) {
  //this will work when the checkbox is ticked for deleting an item
  const checkedItemId = req.body.checkbox;
  const listName = req.body.listName;

  if (listName === "Today") {
    Item.findByIdAndRemove(checkedItemId, function (err) {
      //findByIdAndRemove() is a method that deletes the item of a particular id when checked
      if (!err) {
    ...
    function (err, user) {
      if (err) {
        ...
        passport.authenticate("local")(req, res, function () {
          //type of authentication is local and callback function is triggerred when authentication is a success
          res.redirect("/list");
        });
      }
    }
...
app.post("/login", function (req, res) {
  ...
  req.login(user, function (err) {
    if (err) {
      ...
      passport.authenticate("local")(req, res, function () {
        //if we login successfully we are going to send the cookie and tell our browser to hold on to that cookie, cookie tells that user is authorized
        res.redirect("/list");
      });
    }
  });
